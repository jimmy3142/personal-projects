...
async def lifespan(fastapi_app: FastAPI):
    ...
    yield
    logging.info("Clean up the ML model and release the resources")
    ml_models.clear()

app = FastAPI(title="credit-card-fraud-detection", lifespan=lifespan)
...
def apply_model(input_data: dict[str, int | float]):
    ...
    preds = ml_models["model"].predict(input_data)
    return preds[0]
...
if __name__ == "__main__":
    uvicorn.run("app.predict:app", host="0.0.0.0", port=8000)
